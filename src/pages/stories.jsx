  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // map & sort terbaru dulu (berdasarkan createdAt atau id)
        const mapped = arr
          .map((it) => ({
            id: it.id ?? it.slug,
            slug: it.slug,
          ...
          .sort((a, b) => {
            ...
            // fallback kalau tanggal sama/tidak ada: urut id desc
  ...
  return (
    <div className="min-h-screen bg-[#402923]">
          ...
          <div className="absolute inset-x-0 bottom-0 p-6 md:p-8">
            {loading ? (
              <div className="space-y-3">
                ...
                <div className="h-10 w-3/4 bg-white/40 rounded-md" />
              ...
              <>
                ...
                <h1 className="font-playfair text-2xl md:text-4xl lg:text-5xl font-bold text-white drop-shadow-[0_2px_8px_rgba(0,0,0,0.35)]">
                  <Link to={`/stories/${hero.slug}`}>{hero.title}</Link>
                </h1>
                {hero.summary && (
                  <p className="mt-2 max-w-3xl text-white/90 leading-relaxed">
                    {hero.summary}
                  </p>
                )}
              </>
      ...
      <div className="max-w-6xl mx-auto px-4 pb-20">
        <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-6">
          {(loading ? skeletonCards : list).map((p, i) =>
            loading ? (
              ...
              >
                <div className="w-28 h-24 rounded-lg bg-white/20 animate-pulse" />
              ...
              <article
                key={p.id}
                className="flex gap-4 rounded-2xl bg-white/90 backdrop-blur-sm p-4 shadow-[0_10px_30px_rgba(0,0,0,0.25)] ring-1 ring-black/5 hover:translate-y-[-2px] transition"
              >
                <Link to={`/stories/${p.slug}`} className="shrink-0">
                  <img
                    src={p.image}
                    alt={p.title}
                    className="w-28 h-24 rounded-lg object-cover bg-white/50"
                    loading="lazy"
                    onError={(e) => {
                      e.currentTarget.src = PLACEHOLDER;
                    }}
                ...
                <div className="min-w-0">
                  <p className="text-[13px] text-[#3b241f]/80">
                    {formatDate(p.date)}
                  </p>
                  <h3 className="mt-1 font-poppins font-semibold text-[#1d1d1f] leading-6">
                    <Link
                      to={`/stories/${p.slug}`}
                  ...
                  {p.summary && (
                    <p className="mt-1 text-sm text-[#2b2b2b]/80 line-clamp-3">
                      {p.summary}
                    </p>
                  )}
                </div>
        ...
        {err && (
          <div className="text-center text-red-200 bg-red-900/20 border border-red-400/30 rounded-xl py-3 mt-2">
